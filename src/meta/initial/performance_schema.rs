...
pub fn global_variables_data(global_context: Arc<Mutex<GlobalContext>>) -> MysqlResult<u64> {
    ...
    let engine = engine_util::EngineFactory::try_new(global_context.clone(), meta_const::FULL_TABLE_NAME_OF_DEF_PERFORMANCE_SCHEMA_GLOBAL_VARIABLES.to_object_name(), table_schema.clone());
    match engine {
