...
use sqlparser::ast::{
    ColumnDef as SQLColumnDef, ColumnOption, ColumnOptionDef, DataType as SQLDataType, Ident,
    ObjectName, TableConstraint,
};
...
pub fn get_table_index_list(
    global_context: Arc<Mutex<GlobalContext>>,
    full_table_name: ObjectName,
) -> MysqlResult<Vec<TableIndexDef>> {
...
pub fn create_sql_column(
    column_name: &str,
    data_type: SQLDataType,
    nullable: ColumnOption,
) -> SQLColumnDef {
    SQLColumnDef {
        name: Ident {
...
pub fn create_full_column_name(
    catalog_name: &str,
    schema_name: &str,
    table_name: &str,
    column_name: &str,
...
pub fn fill_up_schema_name(
    session_context: &mut SessionContext,
    mut db_name: ObjectName,
) -> MysqlResult<ObjectName> {
    if db_name.0.len() == 1 {
...
pub fn resolve_table_name(
    ...
    let full_table_name =
        meta_util::fill_up_table_name(session_context, table_name.clone()).unwrap();
...
pub fn fill_up_column_name(
    session_context: &mut SessionContext,
    mut original_column_name: ObjectName,
) -> MysqlResult<ObjectName> {
...
pub fn load_global_variable(global_context: Arc<Mutex<GlobalContext>>) -> MysqlResult<()> {
    let variable_map =
        initial::read_performance_schema_global_variables(global_context.clone()).unwrap();
    global_context
        .lock()
        .unwrap()
        .variable
        .add_variable_map(variable_map);
    Ok(())
}
...
pub async fn init_meta(global_context: Arc<Mutex<GlobalContext>>) -> MysqlResult<()> {
    ...
    init_tables.push(def::performance_schema::global_variables(
    ...
    if full_table_names.len() < 1 {
        for table in init_tables.iter() {
            let result = initial::add_information_schema_tables(
                global_context.clone(),
                table.option.clone(),
            );
            if let Err(e) = result {
            ...
            let result = initial::add_information_schema_columns(
                global_context.clone(),
                table.option.clone(),
                table.column.sparrow_column_list.clone(),
            );
            if let Err(e) = result {
                return Err(e);
            }

            let result = save_table_constraint(
                global_context.clone(),
                table.option.clone(),
                table.get_constraints().clone(),
            );
            if let Err(e) = result {
                return Err(e);
            }
        ...
        let result = initial::create_schema(
            global_context.clone(),
            meta_const::FULL_SCHEMA_NAME_OF_DEF_MYSQL.to_object_name(),
        );
        if let Err(e) = result {
            return Err(e);
        }
        ...
        let result = initial::create_schema(
            global_context.clone(),
            meta_const::FULL_SCHEMA_NAME_OF_DEF_PERFORMANCE_SCHEMA.to_object_name(),
        );
        if let Err(e) = result {
            return Err(e);
        }
...
pub fn schema_name_not_allow_exist(
    global_context: Arc<Mutex<GlobalContext>>,
    session_context: &mut SessionContext,
    table_name: ObjectName,
) -> MysqlResult<()> {
...
pub fn mysql_error_unknown_table(
    global_context: Arc<Mutex<GlobalContext>>,
    session_context: &mut SessionContext,
    table_name: ObjectName,
) -> MysqlResult<()> {
...
pub fn convert_scalar_value_to_string(scalar_value: ScalarValue) -> MysqlResult<Option<String>> {
    match scalar_value {
        ...
        _ => Err(MysqlError::new_global_error(
            ...
            format!(
                "Unsupported convert scalar value to string: {:?}",
                scalar_value
            )
            .as_str(),
        )),
...
pub fn convert_sql_data_type_to_arrow_data_type(sql_type: &SQLDataType) -> MysqlResult<DataType> {
    match sql_type {
        ...
        _ => Err(MysqlError::new_global_error(
            meta_const::MYSQL_ERROR_CODE_UNKNOWN_ERROR,
            format!(
                "Unsupported convert sql data type: {:?} to arrow data type.",
                sql_type
            )
            .as_str(),
        )),
...
pub fn table_has_primary_key(
    global_context: Arc<Mutex<GlobalContext>>,
    full_table_name: ObjectName,
) -> bool {
    let schema_table_constraints =
        initial::read_information_schema_statistics(global_context.clone()).unwrap();
    match schema_table_constraints.get(&full_table_name) {
        Some(vec_table_constraint) => {
            let a = &vec_table_constraint
                .to_vec()
                .iter()
                .any(|table_constraint| match table_constraint {
...
pub fn save_table_constraint(
    ...
    let mut save_create_statistics = SaveStatistics::new(
        global_context.clone(),
        catalog_name.as_str(),
        schema_name.as_str(),
        table_name.as_str(),
    );
    let mut save_key_column_usage = SaveKeyColumnUsage::new(
        global_context.clone(),
        catalog_name.as_str(),
        schema_name.as_str(),
        table_name.as_str(),
    );
    let mut save_table_constraints = SaveTableConstraints::new(
        global_context.clone(),
        catalog_name.as_str(),
        schema_name.as_str(),
        table_name.as_str(),
        ...
        match constraint {
            TableConstraint::Unique {
                ...
                let constraint_name = if is_primary == true {
                    ...
                    match name {
                        Some(n) => n.to_string(),
                        _ => create_unique_index_name(columns.clone()),
                    }
                };
                ...
                for column_name in columns {
                    seq_in_index += 1;
                    save_create_statistics.add_row(
                        constraint_name.as_str(),
                        seq_in_index,
                        column_name.to_string().as_str(),
                    );
                    save_key_column_usage.add_row(
                        constraint_name.as_str(),
                        seq_in_index,
                        column_name.to_string().as_str(),
                    );
...
pub fn cache_add_all_table(global_context: Arc<Mutex<GlobalContext>>) {
    ...
    global_context
        ...
        .unwrap()
        .meta_data
        .add_all_table(all_table);
}

pub fn check_table_exists_with_full_name(
    global_context: Arc<Mutex<GlobalContext>>,
    full_table_name: ObjectName,
) -> MysqlResult<()> {
    let gc = global_context.lock().unwrap();
