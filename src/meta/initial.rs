...
use crate::meta::meta_def::{
    SchemaDef, SchemaOptionDef, SparrowColumnDef, StatisticsColumn, TableColumnDef, TableDef,
    TableOptionDef,
};
...
pub fn create_table(
    ...
    let mut table_option = TableOptionDef::new(meta_const::CATALOG_NAME, schema_name, table_name);
...
pub fn create_schema(
    ...
    for sql_column in def::information_schema::schemata(global_context.clone())
        .column
        .sql_column_list
    {
    ...
    column_value_map.insert(
        "catalog_name".to_ident(),
        ScalarValue::Utf8(Some(catalog_name.to_string())),
    );
    column_value_map.insert(
        "schema_name".to_ident(),
        ScalarValue::Utf8(Some(schema_name.to_string())),
    );
    column_value_map.insert(
        "default_character_set_name".to_ident(),
        ScalarValue::Utf8(Some("utf8mb4".to_string())),
    );
    column_value_map.insert(
        "default_collation_name".to_ident(),
    ...
    column_value_map.insert(
        "DEFAULT_ENCRYPTION".to_ident(),
        ScalarValue::Utf8(Some("NO".to_string())),
    );
...
impl SaveTableConstraints {
    ...
    pub fn add_row(&mut self, constraint_name: &str, constraint_type: &str) {
        let mut column_value_map = HashMap::new();
        column_value_map.insert(
            "constraint_catalog".to_ident(),
            ScalarValue::Utf8(Some(self.catalog_name.clone())),
        );
        column_value_map.insert(
            "constraint_schema".to_ident(),
            ScalarValue::Utf8(Some(self.schema_name.clone())),
        );
        column_value_map.insert(
            "constraint_name".to_ident(),
            ScalarValue::Utf8(Some(constraint_name.to_string())),
        );
        column_value_map.insert(
            "table_schema".to_ident(),
            ScalarValue::Utf8(Some(self.schema_name.clone())),
        );
        column_value_map.insert(
            "table_name".to_ident(),
            ScalarValue::Utf8(Some(self.table_name.clone())),
        );
        column_value_map.insert(
            "constraint_type".to_ident(),
            ScalarValue::Utf8(Some(constraint_type.to_string())),
        );
        column_value_map.insert(
            "enforced".to_ident(),
...
impl SaveKeyColumnUsage {
    ...
    pub fn add_row(&mut self, constraint_name: &str, seq_in_index: i64, column_name: &str) {
        ...
        column_value_map.insert(
            "column_name".to_ident(),
            ScalarValue::Utf8(Some(column_name.to_string())),
        );
        column_value_map.insert(
            "ordinal_position".to_ident(),
            ScalarValue::Int64(Some(seq_in_index)),
        );
        // POSITION_IN_UNIQUE_CONSTRAINT, it is null if constraints is primary or unique
        column_value_map.insert(
            "position_in_unique_constraint".to_ident(),
            ScalarValue::Int64(None),
        );
        column_value_map.insert(
            "referenced_table_schema".to_ident(),
...
impl SaveStatistics {
    ...
    pub fn add_row(&mut self, index_name: &str, seq_in_index: i64, column_name: &str) {
        let mut column_value_map = HashMap::new();
        column_value_map.insert(
            meta_const::COLUMN_NAME_OF_DEF_INFORMATION_SCHEMA_STATISTICS_TABLE_CATALOG.to_ident(),
            ScalarValue::Utf8(Some(self.catalog_name.clone())),
        );
        column_value_map.insert(
            meta_const::COLUMN_NAME_OF_DEF_INFORMATION_SCHEMA_STATISTICS_TABLE_SCHEMA.to_ident(),
            ScalarValue::Utf8(Some(self.schema_name.clone())),
        );
        column_value_map.insert(
            meta_const::COLUMN_NAME_OF_DEF_INFORMATION_SCHEMA_STATISTICS_TABLE_NAME.to_ident(),
            ScalarValue::Utf8(Some(self.table_name.clone())),
        );
        column_value_map.insert(
            meta_const::COLUMN_NAME_OF_DEF_INFORMATION_SCHEMA_STATISTICS_NON_UNIQUE.to_ident(),
            ScalarValue::Int64(Some(0)),
        );
        column_value_map.insert(
            meta_const::COLUMN_NAME_OF_DEF_INFORMATION_SCHEMA_STATISTICS_INDEX_NAME.to_ident(),
            ScalarValue::Utf8(Some(index_name.to_string())),
        );
        column_value_map.insert(
            meta_const::COLUMN_NAME_OF_DEF_INFORMATION_SCHEMA_STATISTICS_SEQ_IN_INDEX.to_ident(),
            ScalarValue::Int64(Some(seq_in_index)),
        );
        column_value_map.insert(
            meta_const::COLUMN_NAME_OF_DEF_INFORMATION_SCHEMA_STATISTICS_COLUMN_NAME.to_ident(),
            ScalarValue::Utf8(Some(column_name.to_string())),
        );
        self.column_value_map_list.push(column_value_map);
    }
    ...
    pub fn save(&mut self) -> MysqlResult<u64> {
        ...
        let mut column_name_list = vec![];
        for column_def in
            def::information_schema::statistics(self.global_context.clone()).get_columns()
        {
            column_name_list.push(column_def.sql_column.name.to_string());
        }
...
pub fn delete_db_form_information_schema(
    global_context: Arc<Mutex<GlobalContext>>,
    full_db_name: ObjectName,
) -> MysqlResult<u64> {
    let full_table_name =
    ...
    let db_name_index = schema_ref
        .index_of(meta_const::COLUMN_NAME_OF_DEF_INFORMATION_SCHEMA_SCHEMATA_SCHEMA_NAME)
        .unwrap();
    let projection = Some(vec![rowid_index, db_name_index]);

    let store_engine = engine_util::StoreEngineFactory::try_new_with_table_name(
        global_context.clone(),
        full_table_name.clone(),
    )
    .unwrap();
    let table_engine = engine_util::TableEngineFactory::try_new_with_table_name(
        global_context.clone(),
        full_table_name.clone(),
    )
    .unwrap();
...
pub fn add_information_schema_tables(
    global_context: Arc<Mutex<GlobalContext>>,
    ...
    let mut column_value_map = HashMap::new();
    column_value_map.insert(
        meta_const::COLUMN_INFORMATION_SCHEMA_TABLE_CATALOG.to_ident(),
        ScalarValue::Utf8(Some(catalog_name.to_string())),
    );
    column_value_map.insert(
        meta_const::COLUMN_INFORMATION_SCHEMA_TABLE_SCHEMA.to_ident(),
        ScalarValue::Utf8(Some(schema_name.to_string())),
    );
    column_value_map.insert(
        meta_const::COLUMN_INFORMATION_SCHEMA_TABLE_NAME.to_ident(),
        ScalarValue::Utf8(Some(table_name.to_string())),
    );
    column_value_map.insert(
        meta_const::COLUMN_NAME_OF_DEF_INFORMATION_SCHEMA_TABLES_TABLE_TYPE.to_ident(),
        ScalarValue::Utf8(Some(table_type.to_string())),
    );
    column_value_map.insert(
        meta_const::COLUMN_NAME_OF_DEF_INFORMATION_SCHEMA_TABLES_ENGINE.to_ident(),
        ScalarValue::Utf8(Some(engine.to_string())),
    );
    column_value_map.insert(
        meta_const::COLUMN_NAME_OF_DEF_INFORMATION_SCHEMA_TABLES_VERSION.to_ident(),
        ScalarValue::Int64(Some(0)),
    );
    column_value_map.insert(
        meta_const::COLUMN_NAME_OF_DEF_INFORMATION_SCHEMA_TABLES_DATA_LENGTH.to_ident(),
        ScalarValue::Int64(Some(0)),
    );
    column_value_map.insert(
        meta_const::COLUMN_NAME_OF_DEF_INFORMATION_SCHEMA_TABLES_INDEX_LENGTH.to_ident(),
        ScalarValue::Int64(Some(0)),
    );
    column_value_map.insert(
        meta_const::COLUMN_NAME_OF_DEF_INFORMATION_SCHEMA_TABLES_AUTO_INCREMENT.to_ident(),
        ScalarValue::Int64(Some(0)),
    );
    column_value_map.insert(
        meta_const::COLUMN_NAME_OF_DEF_INFORMATION_SCHEMA_TABLES_COLUMN_MAX_STORE_ID.to_ident(),
        ScalarValue::Int64(Some(column_max_store_id)),
    );
    column_value_map_list.push(column_value_map);
...
pub fn add_information_schema_columns(
    global_context: Arc<Mutex<GlobalContext>>,
    table_option: TableOptionDef,
    sparrow_column_list: Vec<SparrowColumnDef>,
) -> MysqlResult<u64> {
    ...
    let table_has_primary_key = meta_util::table_has_primary_key(
        global_context.clone(),
        table_option.full_table_name.clone(),
    );

    let mut create_statistics = SaveStatistics::new(
        global_context.clone(),
        catalog_name.as_str(),
        schema_name.as_str(),
        table_name.as_str(),
    ...
    for sparrow_column in sparrow_column_list {
            ...
            match x.option {
                ColumnOption::Unique { is_primary } => {
                    if is_primary {
                        ...
                        create_statistics.add_row(
                            meta_const::NAME_OF_PRIMARY,
                            1,
                            column_name.as_str(),
                        )
                    } else {
        ...
        let data_type =
            meta_util::convert_sql_data_type(&sparrow_column.sql_column.data_type).unwrap();
        let numeric_precision =
            meta_util::get_numeric_precision(&sparrow_column.sql_column.data_type);
        let numeric_scale = meta_util::get_numeric_scale(&sparrow_column.sql_column.data_type);
        let character_maximum_length =
            meta_util::get_character_maximum_length(&sparrow_column.sql_column.data_type);
        let character_octed_length = meta_util::get_character_octed_length();

        let allow_null = sparrow_column
            .sql_column
            .options
            .clone()
            .iter()
        ...
        column_value_map.insert(
            "table_catalog".to_ident(),
            ScalarValue::Utf8(Some(catalog_name.clone())),
        );
        // TABLE_SCHEMA
        column_value_map.insert(
            "table_schema".to_ident(),
            ScalarValue::Utf8(Some(schema_name.clone())),
        );
        // TABLE_NAME
        column_value_map.insert(
            "table_name".to_ident(),
            ScalarValue::Utf8(Some(table_name.clone())),
        );
        // COLUMN_NAME
        column_value_map.insert(
            "column_name".to_ident(),
            ScalarValue::Utf8(Some(column_name.clone())),
        );
        // ORDINAL_POSITION
        column_value_map.insert(
            "store_id".to_ident(),
            ScalarValue::Int64(Some(sparrow_column.store_id)),
        );
        // ORDINAL_POSITION
        column_value_map.insert(
            "ordinal_position".to_ident(),
            ScalarValue::Int64(Some(sparrow_column.ordinal_position)),
        );
        // COLUMN_DEFAULT
        column_value_map.insert("COLUMN_DEFAULT".to_ident(), ScalarValue::Utf8(None));
        // IS_NULLABLE
        column_value_map.insert(
            "is_nullable".to_ident(),
            ScalarValue::Utf8(Some(is_nullable.to_string())),
        );
        // DATA_TYPE
        column_value_map.insert("data_type".to_ident(), ScalarValue::Utf8(Some(data_type)));
        // CHARACTER_MAXIMUM_LENGTH
        column_value_map.insert(
            meta_const::COLUMN_NAME_OF_DEF_INFORMATION_SCHEMA_COLUMNS_CHARACTER_MAXIMUM_LENGTH
                .to_ident(),
            character_maximum_length,
        );
        // CHARACTER_OCTET_LENGTH
        column_value_map.insert(
            meta_const::COLUMN_NAME_OF_DEF_INFORMATION_SCHEMA_COLUMNS_CHARACTER_OCTET_LENGTH
                .to_ident(),
            character_octed_length,
        );
        // NUMERIC_PRECISION
        column_value_map.insert(
            meta_const::COLUMN_NAME_OF_DEF_INFORMATION_SCHEMA_COLUMNS_NUMERIC_PRECISION.to_ident(),
            numeric_precision,
        );
        // NUMERIC_SCALE
        column_value_map.insert(
            meta_const::COLUMN_NAME_OF_DEF_INFORMATION_SCHEMA_COLUMNS_NUMERIC_SCALE.to_ident(),
            numeric_scale,
        );
        // DATETIME_PRECISION
        ...
        // COLUMN_COMMENT
        column_value_map.insert(
            "COLUMN_COMMENT".to_ident(),
            ScalarValue::Utf8(Some("".to_string())),
        );
        // GENERATION_EXPRESSION
        column_value_map.insert(
            "GENERATION_EXPRESSION".to_ident(),
            ScalarValue::Utf8(Some("".to_string())),
        );
        // SRS_ID
...
pub fn read_all_table(
    global_context: Arc<Mutex<GlobalContext>>,
) -> MysqlResult<HashMap<ObjectName, TableDef>> {
    ...
    let schema_table_constraints =
        read_information_schema_statistics(global_context.clone()).unwrap();
...
pub fn read_information_schema_tables(
    global_context: Arc<Mutex<GlobalContext>>,
) -> MysqlResult<HashMap<ObjectName, TableOptionDef>> {
    let table_of_def_information_schema_tables = information_schema::tables(global_context.clone());

    let engine = engine_util::TableEngineFactory::try_new_with_table(
        global_context.clone(),
        table_of_def_information_schema_tables.clone(),
    )
    .unwrap();
    ...
    let column_index_of_db_name = projection_schema
        .index_of(meta_const::COLUMN_NAME_OF_DEF_INFORMATION_SCHEMA_TABLES_TABLE_SCHEMA)
        .unwrap();
    let column_index_of_table_name = projection_schema
        .index_of(meta_const::COLUMN_NAME_OF_DEF_INFORMATION_SCHEMA_TABLES_TABLE_NAME)
        .unwrap();
    let column_index_of_table_type = projection_schema
        .index_of(meta_const::COLUMN_NAME_OF_DEF_INFORMATION_SCHEMA_TABLES_TABLE_TYPE)
        .unwrap();
    let column_index_of_engine = projection_schema
        .index_of(meta_const::COLUMN_NAME_OF_DEF_INFORMATION_SCHEMA_TABLES_ENGINE)
        .unwrap();
    let projection_index_of_column_store_id = projection_schema
        .index_of(meta_const::COLUMN_NAME_OF_DEF_INFORMATION_SCHEMA_TABLES_COLUMN_MAX_STORE_ID)
        .unwrap();
    ...
    loop {
        match table_iterator.next() {
            Some(item) => match item {
                Ok(record_batch) => {
                    let db_name_row: &StringArray =
                        as_string_array(record_batch.column(column_index_of_db_name));
                    let table_name_row: &StringArray =
                        as_string_array(record_batch.column(column_index_of_table_name));
                    let table_type_row: &StringArray =
                        as_string_array(record_batch.column(column_index_of_table_type));
                    let engine_row: &StringArray =
                        as_string_array(record_batch.column(column_index_of_engine));
                    let column_store_id_row: &Int64Array = as_primitive_array(
                        record_batch.column(projection_index_of_column_store_id),
                    );
...
pub fn read_information_schema_schemata(
    global_context: Arc<Mutex<GlobalContext>>,
) -> MysqlResult<HashMap<ObjectName, SchemaDef>> {
    let table_of_information_schema_schemata = information_schema::schemata(global_context.clone());

    let engine = engine_util::TableEngineFactory::try_new_with_table(
        global_context.clone(),
        table_of_information_schema_schemata.clone(),
    )
    .unwrap();
    let mut table_iterator = engine.table_iterator(None, &[]);
    ...
    let column_index_of_db_name = projection_schema
        .index_of(meta_const::COLUMN_NAME_OF_DEF_INFORMATION_SCHEMA_SCHEMATA_SCHEMA_NAME)
        .unwrap();
    let column_index_of_default_character_set_name = projection_schema
        .index_of(
            meta_const::COLUMN_NAME_OF_DEF_INFORMATION_SCHEMA_SCHEMATA_DEFAULT_CHARACTER_SET_NAME,
        )
        .unwrap();
    let column_index_of_default_collation_name = projection_schema
        .index_of(meta_const::COLUMN_NAME_OF_DEF_INFORMATION_SCHEMA_SCHEMATA_DEFAULT_COLLATION_NAME)
        .unwrap();
    ...
    loop {
        match table_iterator.next() {
            Some(item) => match item {
                Ok(record_batch) => {
                    let column_of_db_name: &StringArray =
                        as_string_array(record_batch.column(column_index_of_db_name));
                    let column_of_default_character_set_name: &StringArray = as_string_array(
                        record_batch.column(column_index_of_default_character_set_name),
                    );
                    let column_of_default_collation_name: &StringArray = as_string_array(
                        record_batch.column(column_index_of_default_collation_name),
                    );

                    for row_index in 0..record_batch.num_rows() {
                        let schema_name = column_of_db_name.value(row_index).to_string();
                        let default_character_set_name = column_of_default_character_set_name
                            .value(row_index)
                            .to_string();
                        let default_collation_name = column_of_default_collation_name
                            .value(row_index)
                            .to_string();

                        let mut schema_option =
                            SchemaOptionDef::new(meta_const::CATALOG_NAME, schema_name.as_str());
                        schema_option
                            .with_default_character_set_name(default_character_set_name.as_str());
                        schema_option.with_default_collation_name(default_collation_name.as_str());

                        let full_schema_name = meta_util::create_full_schema_name(
                            meta_const::CATALOG_NAME,
                            schema_name.as_str(),
                        );
...
pub fn read_information_schema_statistics(
    global_context: Arc<Mutex<GlobalContext>>,
) -> MysqlResult<HashMap<ObjectName, Vec<TableConstraint>>> {
    ...
    let column_index_of_db_name = projection_schema
        .index_of(meta_const::COLUMN_NAME_OF_DEF_INFORMATION_SCHEMA_STATISTICS_TABLE_SCHEMA)
        .unwrap();
    let column_index_of_table_name = projection_schema
        .index_of(meta_const::COLUMN_NAME_OF_DEF_INFORMATION_SCHEMA_STATISTICS_TABLE_NAME)
        .unwrap();
    let column_index_of_index_name = projection_schema
        .index_of(meta_const::COLUMN_NAME_OF_DEF_INFORMATION_SCHEMA_STATISTICS_INDEX_NAME)
        .unwrap();
    let column_index_of_seq_in_index = projection_schema
        .index_of(meta_const::COLUMN_NAME_OF_DEF_INFORMATION_SCHEMA_STATISTICS_SEQ_IN_INDEX)
        .unwrap();
    let column_index_of_column_name = projection_schema
        .index_of(meta_const::COLUMN_NAME_OF_DEF_INFORMATION_SCHEMA_STATISTICS_COLUMN_NAME)
        .unwrap();

    let mut schema_table_index: HashMap<ObjectName, HashMap<String, Vec<StatisticsColumn>>> =
        HashMap::new();
    loop {
        match table_iterator.next() {
            Some(item) => match item {
                Ok(record_batch) => {
                    let column_of_db_name: &StringArray =
                        as_string_array(record_batch.column(column_index_of_db_name));
                    let column_of_table_name: &StringArray =
                        as_string_array(record_batch.column(column_index_of_table_name));
                    let column_of_index_name: &StringArray =
                        as_string_array(record_batch.column(column_index_of_index_name));
                    let column_of_seq_in_index: &Int64Array =
                        as_primitive_array(record_batch.column(column_index_of_seq_in_index));
                    let column_of_column_name: &StringArray =
                        as_string_array(record_batch.column(column_index_of_column_name));
    ...
    for (schema_name, index_field) in schema_table_index.iter() {
        ...
        for (index_name, column_field) in index_field.iter() {
            ...
            let columns = column_field
                .iter()
                .map(|statistics_column| Ident {
                    value: statistics_column.column_name.clone(),
                    quote_style: None,
                })
                .collect::<Vec<_>>();
        ...
        schema_table_constraint
            ...
            .or_insert(table_constraints);
    }

    Ok(schema_table_constraint.clone())
}

pub fn read_information_schema_columns(
    global_context: Arc<Mutex<GlobalContext>>,
) -> MysqlResult<HashMap<ObjectName, Vec<SparrowColumnDef>>> {
    ...
    let column_index_of_db_name = projection_schema
        .index_of(meta_const::COLUMN_NAME_OF_DEF_INFORMATION_SCHEMA_COLUMNS_TABLE_SCHEMA)
        .unwrap();
    let column_index_of_table_name = projection_schema
        .index_of(meta_const::COLUMN_NAME_OF_DEF_INFORMATION_SCHEMA_COLUMNS_TABLE_NAME)
        .unwrap();
    let column_index_of_column_name = projection_schema
        .index_of(meta_const::COLUMN_NAME_OF_DEF_INFORMATION_SCHEMA_COLUMNS_COLUMN_NAME)
        .unwrap();
    let projection_index_of_store_id = projection_schema
        .index_of(meta_const::COLUMN_NAME_OF_DEF_INFORMATION_SCHEMA_COLUMNS_STORE_ID)
        .unwrap();
    let column_index_of_ordinal_position = projection_schema
        .index_of(meta_const::COLUMN_NAME_OF_DEF_INFORMATION_SCHEMA_COLUMNS_ORDINAL_POSITION)
        .unwrap();
    let column_index_of_is_nullable = projection_schema
        .index_of(meta_const::COLUMN_NAME_OF_DEF_INFORMATION_SCHEMA_COLUMNS_IS_NULLABLE)
        .unwrap();
    let column_index_of_data_type = projection_schema
        .index_of(meta_const::COLUMN_NAME_OF_DEF_INFORMATION_SCHEMA_COLUMNS_DATA_TYPE)
        .unwrap();
    let column_index_of_character_maximum_length = projection_schema
        .index_of(
            meta_const::COLUMN_NAME_OF_DEF_INFORMATION_SCHEMA_COLUMNS_CHARACTER_MAXIMUM_LENGTH,
        )
        .unwrap();
    let column_index_of_numeric_precision = projection_schema
        .index_of(meta_const::COLUMN_NAME_OF_DEF_INFORMATION_SCHEMA_COLUMNS_NUMERIC_PRECISION)
        .unwrap();
    let column_index_of_numeric_scale = projection_schema
        .index_of(meta_const::COLUMN_NAME_OF_DEF_INFORMATION_SCHEMA_COLUMNS_NUMERIC_SCALE)
        .unwrap();
        ...
        match table_iterator.next() {
            Some(item) => match item {
                Ok(record_batch) => {
                    ...
                    let column_of_column_name: &StringArray =
                        as_string_array(record_batch.column(column_index_of_column_name));
                    let store_id_row: &Int64Array =
                        as_primitive_array(record_batch.column(projection_index_of_store_id));
                    let column_of_ordinal_position: &Int64Array =
                        as_primitive_array(record_batch.column(column_index_of_ordinal_position));
                    let column_of_is_nullable: &StringArray =
                        as_string_array(record_batch.column(column_index_of_is_nullable));
                    let column_of_data_type: &StringArray =
                        as_string_array(record_batch.column(column_index_of_data_type));
                    let column_of_character_maximum_length: &Int64Array = as_primitive_array(
                        record_batch.column(column_index_of_character_maximum_length),
                    );
                    let column_of_numeric_precision: &Int64Array =
                        as_primitive_array(record_batch.column(column_index_of_numeric_precision));
                    let column_of_numeric_scale: &Int64Array =
                        as_primitive_array(record_batch.column(column_index_of_numeric_scale));
                    ...
                    for row_index in 0..record_batch.num_rows() {
                        ...
                        let character_maximum_length =
                            column_of_character_maximum_length.value(row_index);
                        ...
                        let sql_data_type =
                            meta_util::create_sql_data_type(data_type.as_str()).unwrap();
                        let nullable = meta_util::text_to_null(is_nullable.as_str()).unwrap();

                        let sql_column = meta_util::create_sql_column(
                            column_name.as_str(),
                            sql_data_type,
                            nullable,
                        );
                        let sparrow_column = meta_util::create_sparrow_column(
                            store_id,
                            ordinal_position,
                            sql_column,
                        );
...
pub fn read_performance_schema_global_variables(
    global_context: Arc<Mutex<GlobalContext>>,
) -> MysqlResult<HashMap<String, String>> {
    ...
    let projection_schema =
        performance_schema::global_variables(global_context.clone()).to_schema();

    let column_index_of_variable_name = projection_schema
        .index_of(meta_const::COLUMN_NAME_OF_DEF_PERFORMANCE_SCHEMA_GLOBAL_VARIABLES_VARIABLE_NAME)
        .unwrap();
    let column_index_of_variable_value = projection_schema
        .index_of(meta_const::COLUMN_NAME_OF_DEF_PERFORMANCE_SCHEMA_GLOBAL_VARIABLES_VARIABLE_VALUE)
        .unwrap();
    ...
    loop {
        match table_iterator.next() {
            Some(item) => match item {
                Ok(record_batch) => {
                    let column_of_variable_name: &StringArray =
                        as_string_array(record_batch.column(column_index_of_variable_name));
                    let column_of_variable_value: &StringArray =
                        as_string_array(record_batch.column(column_index_of_variable_value));
...
pub fn add_def_mysql_users(global_context: Arc<Mutex<GlobalContext>>) -> MysqlResult<u64> {
    ...
    // Select_priv
    column_value_map.insert(
        "Select_priv".to_ident(),
        ScalarValue::Utf8(Some("Y".to_string())),
    );
    // Insert_priv
    column_value_map.insert(
        "Insert_priv".to_ident(),
        ScalarValue::Utf8(Some("Y".to_string())),
    );
    // Update_priv
    column_value_map.insert(
        "Update_priv".to_ident(),
        ScalarValue::Utf8(Some("Y".to_string())),
    );
    // Delete_priv
    column_value_map.insert(
        "Delete_priv".to_ident(),
        ScalarValue::Utf8(Some("Y".to_string())),
    );
    // Create_priv
    column_value_map.insert(
        "Create_priv".to_ident(),
        ScalarValue::Utf8(Some("Y".to_string())),
    );
    // Drop_priv
    column_value_map.insert(
    ...
    // Reload_priv
    column_value_map.insert(
        "Reload_priv".to_ident(),
        ScalarValue::Utf8(Some("Y".to_string())),
    );
    // Shutdown_priv
    column_value_map.insert(
        "Shutdown_priv".to_ident(),
        ScalarValue::Utf8(Some("Y".to_string())),
    );
    // Process_priv
    column_value_map.insert(
        "Process_priv".to_ident(),
        ScalarValue::Utf8(Some("Y".to_string())),
    );
    // File_priv
    column_value_map.insert(
    ...
    column_value_map.insert(
        "Create_tmp_table_priv".to_ident(),
        ScalarValue::Utf8(Some("Y".to_string())),
    );
    // Lock_tables_priv
    column_value_map.insert(
        "Lock_tables_priv".to_ident(),
        ScalarValue::Utf8(Some("Y".to_string())),
    );
    // Execute_priv
    column_value_map.insert(
        "Execute_priv".to_ident(),
        ScalarValue::Utf8(Some("Y".to_string())),
    );
    // Repl_slave_priv
    column_value_map.insert(
        "Repl_slave_priv".to_ident(),
        ScalarValue::Utf8(Some("Y".to_string())),
    );
    // Repl_client_priv
    column_value_map.insert(
        "Repl_client_priv".to_ident(),
        ScalarValue::Utf8(Some("Y".to_string())),
    );
    // Create_view_priv
    column_value_map.insert(
        "Create_view_priv".to_ident(),
        ScalarValue::Utf8(Some("Y".to_string())),
    );
    // Show_view_priv
    column_value_map.insert(
        "Show_view_priv".to_ident(),
        ScalarValue::Utf8(Some("Y".to_string())),
    );
    // Create_routine_priv
    column_value_map.insert(
        "Create_routine_priv".to_ident(),
        ScalarValue::Utf8(Some("Y".to_string())),
    );
    // Alter_routine_priv
    column_value_map.insert(
        "Alter_routine_priv".to_ident(),
        ScalarValue::Utf8(Some("Y".to_string())),
    );
    // Create_user_priv
    column_value_map.insert(
        "Create_user_priv".to_ident(),
        ScalarValue::Utf8(Some("Y".to_string())),
    );
    // Event_priv
    column_value_map.insert(
    ...
    // Trigger_priv
    column_value_map.insert(
        "Trigger_priv".to_ident(),
        ScalarValue::Utf8(Some("Y".to_string())),
    );
    // Create_tablespace_priv
    column_value_map.insert(
        "Create_tablespace_priv".to_ident(),
        ScalarValue::Utf8(Some("Y".to_string())),
    );
    // ssl_type
    ...
    column_value_map.insert(
        "max_user_connections".to_ident(),
        ScalarValue::Int64(Some(0)),
    );
    // plugin
    column_value_map.insert(
        "plugin".to_ident(),
        ScalarValue::Utf8(Some("mysql_native_password".to_string())),
    );
    // authentication_string
    column_value_map.insert(
        "authentication_string".to_ident(),
        ScalarValue::Utf8(Some(
            "*6BB4837EB74329105EE4568DDA7DC67ED2CA2AD9".to_string(),
        )),
    );
    // password_expired
    column_value_map.insert(
        "password_expired".to_ident(),
    ...
    // account_locked
    column_value_map.insert(
        "account_locked".to_ident(),
        ScalarValue::Utf8(Some("N".to_string())),
    );
    // Create_role_priv
    column_value_map.insert(
        "Create_role_priv".to_ident(),
        ScalarValue::Utf8(Some("Y".to_string())),
    );
    // Drop_role_priv
    column_value_map.insert(
        "Drop_role_priv".to_ident(),
        ScalarValue::Utf8(Some("Y".to_string())),
    );
    ...
    column_value_map.insert(
        "Password_require_current".to_ident(),
        ScalarValue::Utf8(None),
    );
    // User_attributes
...
pub fn add_def_performance_schmea_global_variables(
    ...
    column_value_map.insert(
        ...
        ScalarValue::Utf8(Some("auto_increment_increment".to_string())),
    );
    column_value_map.insert(
    ...
    column_value_map.insert(
        "variable_name".to_ident(),
        ScalarValue::Utf8(Some("lower_case_table_names".to_string())),
    );
    column_value_map.insert(
        "variable_value".to_ident(),
    ...
    column_value_map.insert(
        "variable_name".to_ident(),
        ScalarValue::Utf8(Some("transaction_read_only".to_string())),
    );
    column_value_map.insert(
        "variable_value".to_ident(),
        ScalarValue::Utf8(Some("0".to_string())),
...
pub fn get_full_table_name_list(
    global_context: Arc<Mutex<GlobalContext>>,
) -> MysqlResult<Vec<ObjectName>> {
    let meta_table = information_schema::tables(global_context.clone());

    let engine =
        TableEngineFactory::try_new_with_table(global_context.clone(), meta_table).unwrap();
    let mut table_iterator = engine.table_iterator(None, &[]);
