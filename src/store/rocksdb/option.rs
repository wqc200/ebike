...
use std::mem;
use super::ffi;
use libc::{self, c_int, c_uchar, c_uint, c_char, c_void};
use libc::{size_t};
...
use super::db::DB;
use super::slice_transform::SliceTransform;
use super::db::{DBCompressionType, DBCompactionStyle};
use super::{merge_operator, compaction_filter, comparator};
use super::merge_operator::{partial_merge_callback, full_merge_callback, MergeFn, MergeOperatorCallback};
use super::compaction_filter::{filter_callback, CompactionFilterFn, CompactionFilterCallback};
use super::comparator::{CompareFn, ComparatorCallback};
...
/// use sparrow::rocksdb::db::{DB, DBCompactionStyle};
...
///
/// ```
/// use sparrow::rocksdb::option::{FlushOptions, Options};
...
///
/// use sparrow::rocksdb::db::DB;
/// use sparrow::rocksdb::option::{WriteOptions, Options};
/// let path = "_path_for_rocksdb_storageY";
/// {
///     let db = DB::open_default(path).unwrap();
...
impl BlockBasedOptions {
    ...
    ///
    /// ```
    /// use sparrow::rocksdb::option::{Options, BlockBasedOptions, BlockBasedIndexType};
    ///
    /// let mut opts = Options::default();
...
impl Options {
    ...
    /// ```
    /// use sparrow::rocksdb::option::Options;
    /// use sparrow::rocksdb::db::DBCompressionType;
    ///
    /// let mut opts = Options::default();
    ...
    /// ```
    /// use sparrow::rocksdb::option::Options;
    /// use sparrow::rocksdb::db::DBCompactionStyle;
    ///
    /// let mut opts = Options::default();
    /// opts.set_compaction_style(DBCompactionStyle::Universal);
    /// ```
    ...
    /// use sparrow::rocksdb::option::{Options, MemtableFactory};
    /// let mut opts = Options::default();
    ...
    ///
    /// ```
    /// use sparrow::rocksdb::option::{Options, PlainTableFactoryOptions};
    ///
    /// let mut opts = Options::default();
    ...
    ///
    /// ```
    /// use sparrow::rocksdb::option::{Options, DBRecoveryMode};
    ///
    /// let mut opts = Options::default();
    ...
    /// ```
    /// use sparrow::rocksdb::option::Options;
    /// use sparrow::rocksdb::slice_transform::SliceTransform;
    ///
    /// let mut opts = Options::default();
...
impl FlushOptions {
    ...
    ///
    /// ```
    /// use sparrow::rocksdb::option::FlushOptions;
    ///
