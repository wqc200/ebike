...
use crate::meta::meta_def::{SparrowColumnDef, SchemaDef, TableDef};
...
impl DropSchema {
    ...
    pub async fn execute(&mut self, origin_schema_name: ObjectName) -> MysqlResult<u64> {
        let full_schema_name =
            meta_util::fill_up_table_name(&mut self.session_context, origin_schema_name.clone()).unwrap();
        ...
        let result = meta_util::get_schema(self.global_context.clone(), full_schema_name.clone());
        let schema_def = match result {
            Ok(schema_def) => schema_def.clone(),
            Err(mysql_error) => return Err(mysql_error),
        };

        // delete from information_schema.schemata
        let result = self.delete_from_information_schema_schemata(schema_def.clone()).await;
    ...
    async fn delete_from_information_schema_schemata(&mut self, schema_def: SchemaDef) -> MysqlResult<u64> {
        ...
        let selection = core_util::selection_information_schema_schemata(
            schema_def.option.catalog_name.as_ref(),
            schema_def.option.schema_name.as_str(),
        );
